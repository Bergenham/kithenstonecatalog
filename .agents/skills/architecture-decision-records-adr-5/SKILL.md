---
name: architecture-decision-records-adr-5
description: "Documents significant architectural decisions using standardized Nygard templates to preserve technical context and trade-offs."
---
...
- **`scripts/create-adr.md`** - Dynamic ADR generator with auto-filled context Auto-fills: ADR number, date, author, total ADRs count Usage: `/create-adr [number] [title]` Uses `$ARGUMENTS` and `!command` for dynamic context
